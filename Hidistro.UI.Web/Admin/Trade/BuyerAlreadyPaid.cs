	...
	public class BuyerAlreadyPaid : AdminPage
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			else if (a == "GetStype")
			{
				...
				int num2 = allOrderID.Select(string.Concat(new object[]
				{
					"OrderStatus=",
					2,
					" or (OrderStatus=",
				...
				int countOrderIDByStatus = OrderHelper.GetCountOrderIDByStatus(new OrderStatus?(OrderStatus.BuyerAlreadyPaid), new OrderStatus?(OrderStatus.ApplyForRefund));
				int num4 = allOrderID.Select(string.Concat(new object[]
				{
					...
					" or (OrderStatus=",
					1,
					" AND Gateway = 'hishop.plugins.payment.podrequest')) and IsPrinted=0"
				})).Length;
				int num5 = allOrderID.Select(string.Concat(new object[]
				{
					"(OrderStatus=",
					2,
					" or (OrderStatus=",
					1,
					" AND Gateway = 'hishop.plugins.payment.podrequest')) and IsPrinted=1"
				})).Length;
				string s = string.Concat(new object[]
				{
					"{\"type\":\"1\",\"buyalreadypaidcount\":",
					num2,
					",\"count1\":",
					num3,
					",\"count2\":",
			...
			if (orderInfo != null)
			{
				if (e.CommandName == "CONFIRM_PAY" && orderInfo.CheckAction(OrderActions.SELLER_CONFIRM_PAY))
				{
					...
					if (OrderHelper.ConfirmPay(orderInfo))
					{
						...
						orderInfo.PayDate = new System.DateTime?(System.DateTime.Now);
						this.BindOrders();
						orderInfo.OnPayment();
		...
		private OrderQuery GetOrderQuery()
		{
			...
			{
			case 1:
				orderQuery.PaymentType = new int?(0);
				break;
			case 2:
				orderQuery.OrderItemsStatus = new OrderStatus?(OrderStatus.ApplyForRefund);
				break;
			case 3:
				orderQuery.IsPrinted = new int?(0);
				break;
			case 4:
				orderQuery.IsPrinted = new int?(1);
				break;
			}
			if (!string.IsNullOrEmpty(this.Page.Request.QueryString["ModeId"]))
			{
				int value = 0;
				if (int.TryParse(this.Page.Request.QueryString["ModeId"], out value))
				{
					orderQuery.ShippingModeId = new int?(value);
				}
			}
			int value2;
			if (!string.IsNullOrEmpty(this.Page.Request.QueryString["region"]) && int.TryParse(this.Page.Request.QueryString["region"], out value2))
			{
				orderQuery.RegionId = new int?(value2);
			}
			int value3;
			if (!string.IsNullOrEmpty(this.Page.Request.QueryString["UserId"]) && int.TryParse(this.Page.Request.QueryString["UserId"], out value3))
			{
				orderQuery.UserId = new int?(value3);
			}
			int num = 0;
