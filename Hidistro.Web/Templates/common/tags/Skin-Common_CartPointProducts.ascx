...
<asp:Repeater ID="rptCartPointProduct" runat="server" DataSource='<%# Eval("LineItems") %>' >
    <ItemTemplate>
                ...
                <div class="info">
                    ...
                    <div class="goods-num">
		                ...
		                <input type="tel" class="ui_textinput" name="buyNum" value='<%# Eval("Quantity")%>'
		                    skuid='<%# Eval("SkuId")%>' stype='<%# Eval("Type")%>' exchangeId='<%# Eval("ExchangeId")%>' />
		                <div name="spAdd" class="shopcart-add">
		                    + 
		                </div>
		            </div>
                </div>
            <a href="javascript:void(0)" name="iDelete" skuid='<%# Eval("SkuId")%>' stype='<%# Eval("Type")%>'><span
