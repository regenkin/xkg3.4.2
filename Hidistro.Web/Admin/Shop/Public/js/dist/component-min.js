...
HiShop.popbox.CouponPicker = function (a, b) {
    var c = $("#tpl_popbox_CouponPicker").html(),
    d = $(c),
    e = {
        1: [],
        2: [],
    ...
    f = function (a, b, c) {
        var g = function (b) {
            if (e[a] = b.list, e[a] && e[a].length) {
                var g = $("#tpl_popbox_CouponPicker_item").html(),
                h = _.template(g, {
                    dataset: e[a]
                }),
                i = $(h);
...
HiShop.popbox.CategoriesPicker = function (a, b) {
    ...
    i = function (a, b) {
        var d = function (a) {
            if (c = a.list, c && c.length) {
                ...
                j = $(e);
                j.find(".selectitem").click(function () {
                    var a, b = $(this),
                    d = b.parent("p"),
                    e = d.index(),
                    f = d.data("item"),
            ...
            f.find("li").each(function (a, b) {
                ...
                $.each(m,
                function (a, b) {
                    d == b && (c.addClass("selected"), c.children(".selectitem").addClass("btn-success").text("已选"))
                })
            }),
            b && b()
        };
    ...
    l = function (a) {
        var d = 3;
        f.find(".j-btn-goodsuse").remove(),
        f.on("click", ".selectitem",
        function () {
            var item = [];
            item.title = $(this).attr("title");
            item.link = $(this).attr("datalink");
            var e = ($(".modalshopclasslistbox p").index($(this).parent("p")) - 1) / 2;
            b && b(item, d), $.jBox.close(a)
        })
    }
    switch (a) {
        case "all":
                    ...
                    onOpen: function (b) {
                        if ("goodsMulti" == a) {
                            ...
                            f.find("li").each(function (a, b) {
                                ...
                                $.each(d,
                                function (a, b) {
                                    e == b && (c.addClass("selected"), c.children(".selectitem").addClass("btn-success").text("已选"))
                                })
                            }),
                            k(b, d)
                        } else l(b);
...
function (a, b, c) {
    ...
    },/*z是新增的参数（kuaiwei），用来区分相册的类别，1代表图标*/
    a.albums = function (c, z) {
        ...
        function () {
            ...
            return a.ajax({
                ...
                success: function (a) {
                    1 == a.status ? (b.closest(".albums-edit").children(".img-name-edit").hide(), b.closest(".albums-edit").children("p").html(d), b.closest(".albums-edit").children("input.file_name").val(d), HiShop.hint("success", "恭喜您，操作成功！")) : HiShop.hint("danger", a.msg)//"对不起，操作失败"
                }
            }),
            !1
        }),
...
HiShop.popbox.ImgPicker = function (a, b) {
    $.albums({
        callback: a
    }, b)
    if ($(".nav-tabs li[class='active']").attr("type") != "0") {
        $("#j-addImg").hide();
    }
};