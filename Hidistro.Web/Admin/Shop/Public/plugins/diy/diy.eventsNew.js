$(function () {
    ...
    HiShop.DIY.Unit.event_type2 = function (ctrldom, data) {
        ...
        //导航颜色选择器
        $ctrl.find(".colorPicker").each(function (index,e) {
            if (index == 0) {
    ...
    HiShop.DIY.Unit.event_type5 = function (ctrldom, data) {
        ...
        //是否显示购物车图标
        $ctrl.find("input[name='showMaketPrice']").change(function () {
            var val = $(this).is(":checked");
            data.content.showMaketPrice = val;//同步数据到缓存
            reRender();
        });
        ...
        $ctrl.find("select[name='thirdPriority']").change(function () {
            ...
            HiShop.Goods.GoodsEvents(data, function (json) {
                json.content.firstPriority = data.content.firstPriority;
                json.content.secondPriority = data.content.secondPriority;
                json.content.showMaketPrice = data.content.showMaketPrice;
    ...
    HiShop.DIY.Unit.event_type10 = function (ctrldom, data) {
        ...
        $ctrl.find("input[name='style']").change(function () {
            var val = $(this).val();
            data.content.style = val;//同步数据到缓存
            if (data.content.style == 0) {
                data.content.styleClass = "custom-line";
            } else {
                data.content.styleClass = "solid-line";
            }
            reRender();
        });
        //导航颜色选择器
    ...
    HiShop.DIY.Unit.event_type13 = function (ctrldom, data) {
        ...
        $ctrl.find('input[name="layout"]').change(function (event) {
            ...
            if (data.content.layout == "1" && data.content.dataset[0].pic.indexOf("/Admin/shop/Public/images/diy/waitupload") > -1) {
                if (data.content.dataset[0].pic.indexOf("/Admin/shop/Public/images/diy/waitupload") > -1) {
                    data.content.dataset[0].pic = "/Admin/shop/Public/images/diy/waitupload.png";
                }
            } else {
                if (data.content.dataset[0].pic.indexOf("/Admin/shop/Public/images/diy/waitupload") > -1) {
                    data.content.dataset[0].pic = "/Admin/shop/Public/images/diy/waitupload1.png";
                }
            }
                        ...
                        $render.find('.j-getAudioName').click(function (event) {
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    ...
                                    if (data.status == 1) {
                                        HiShop.hint("success", "恭喜您，修改音频名称成功！");
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.status == 1) {
                            ...
                            HiShop.hint("danger", "对不起，获取数据失败：" + data.msg);
                        }
                        // $.jBox.hideloading();
                    }
                });
            ...
            var initUpload = function (jbox) {
                ...
                $picker.find("#imgpicker_upload_input").uploadify({
                    ...
                    'onSelectError': function (file, errorCode, errorMsg) {
                        switch (errorCode) {
                            case -100:
                                ...
                                break;
                            case -110:
                                HiShop.hint("danger", "对不起，文件 [" + file.name + "] 大小超出5MB！");
                                break;
                            case -120:
                                HiShop.hint("danger", "文件 [" + file.name + "] 大小异常！");
                                break;
                            case -130:
                                HiShop.hint("danger", "文件 [" + file.name + "] 类型不正确！");
                                break;
                        }
                    },
                    'onFallback': function () {
                    ...
                    onUploadError: function (file, errorCode, errorMsg, errorString) {
                        HiShop.hint("danger", "对不起：" + file.name + "上传失败：" + errorString);
                    }
                });
                ...
                $picker.find("#j-btn-uploaduse").click(function () {
                    if (arrSelected.length == 0) {
                        HiShop.hint("danger", "对不起，您没有选择音频：" + data.msg);
                    } else {
    ...
    //模块类型16 公告
    HiShop.DIY.Unit.event_type16 = function (ctrldom, data) {
        var $conitem = data.dom_conitem, //手机内容
            $ctrl = ctrldom; //控制内容
        ...
        $ctrl.find("input[name='notice']").change(function () {
            var val = $(this).val();
            $conitem.find(".j-notice").text("公告：" + $(this).val());
            if (val.length > 16) {
                $conitem.find("marquee").show().siblings('h2').hide();
            } else {
                $conitem.find(".j-notice").show().siblings('marquee').hide();
            }
            data.content.notice = val;
            reRender();
        });
    };
});