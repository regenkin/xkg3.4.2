<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/AdminNew.Master" AutoEventWireup="true" CodeBehind="OrderDetails.aspx.cs" Inherits="Hidistro.UI.Web.Admin.Trade.OrderDetails" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <form runat="server">
        ...
        <asp:HiddenField ID="hdReturnsId" runat="server" />
         <asp:HiddenField ID="hdOrderItemId" runat="server" />
                        ...
                        <ItemTemplate>
                            <tr>
                                ...
                                <td style="border-left: 1px solid #ccc; padding: 15px; vertical-align: middle; width: 395px;">
                                    ...
                                    <p>
                                        <input type="button" class="btn btn-success btn-sm inputw100" value="同意退款" id="btnAgree" runat="server" returnsid='<%#Eval("ReturnsId") %>' skuid='<%#Eval("skuid") %>' orderitemid='<%#Eval("ID") %>' productid='<%#Eval("productid") %>' money='<%#Eval("RefundMoney","{0:F2}") %>' onclick="AgreeReturnFun($(this).attr('productid'), $(this).attr('skuid'), $(this).attr('money'), $(this).attr('returnsid'), $(this).attr('orderitemid'))" />
                                        <input type="button" class="btn btn-danger btn-sm inputw100" value="拒绝退款" id="btnRefuce" runat="server" returnsid='<%#Eval("ReturnsId") %>' skuid='<%#Eval("skuid") %>' orderitemid='<%#Eval("ID") %>' productid='<%#Eval("productid") %>' onclick="RefuseReturnFun($(this).attr('productid'), $(this).attr('skuid'), $(this).attr('returnsid'), $(this).attr('orderitemid'))" />
        ...
        <div class="modal fade" id="closeOrder">
            <div class="modal-dialog">
                <div class="modal-content form-horizontal">
                    ...
                    <div class="modal-footer">
                        <asp:Button ID="btnCloseOrder" runat="server" CssClass="btn btn-success" Text="关闭订单" OnClientClick="return checkClose()" />
                        <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                    </div>
                </div>
            </div>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $(".cssModifyRefundMoney").click(function(){
                ...
                var maxprice = parseFloat($("#ctl00_ContentPlaceHolder1_lblorderTotalForRemark").html());
                var data = "posttype=modifyRefundMondy&price=" + price + "&oid=" + oid + "&pid=" + pid;
                if (price > maxprice) {
                    ShowMsg("退款金额不能大于实付款" + maxprice + "元！");
                } else {
                    $.ajax({
                        type: "post",
        ...
        /*同意退款*/
        function AgreeReturnFun(productid,skuid,money,returnsid,itemid){
            ...
            $("#ctl00_ContentPlaceHolder1_hdOrderItemId").val(itemid);
        ...
        /*拒绝退款*/
        function RefuseReturnFun(productid, skuid, returnsid, itemid) {
            $("#ctl00_ContentPlaceHolder1_hdProductID").val(productid);
            $("#ctl00_ContentPlaceHolder1_hdSkuID").val(skuid);
            $("#ctl00_ContentPlaceHolder1_hdReturnsId").val(returnsid);
            $("#ctl00_ContentPlaceHolder1_hdOrderItemId").val(itemid);
