<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/AdminNew.Master" AutoEventWireup="true" CodeBehind="ReplyEdit.aspx.cs" Inherits="Hidistro.UI.Web.Admin.WeiXin.ReplyEdit" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <form id="aspnetForm" runat="server">
        <div class="page-header">
            <h2><%=htmlTitle %></h2>
            <small>用户关注商户微信号后，系统自动发送给用户的消息。若回复内容为空，则用户关注该商户微信号后，不会收到任何回复消息。</small>
        </div>
            ...
            <div class="fl frwidth">
                <div class="set-switch resetBorder" style="display:<%=(type=="subscribe")?"none":"block"%>">
                    ...
                    <div class="resetradio mb10">
                        <asp:RadioButtonList ID="rbtlMatchType" runat="server" onclick="SetVisible()" RepeatDirection="Horizontal" Width="300">
                            <asp:ListItem Text="精确匹配" Value="2"></asp:ListItem>
                            <asp:ListItem Text="模糊匹配" Value="1"></asp:ListItem>
                            <asp:ListItem Text="无匹配回复" Value="4"></asp:ListItem>
                        </asp:RadioButtonList>
                    </div>
                    <small id="hint2">精确匹配是对话完全一致才能触发，模糊匹配是包含就触发。</small>
                    <p class="mb10"><strong>关键词：</strong></p>
                    <asp:TextBox ID="txtKeys" runat="server" MaxLength="200" CssClass="form-control mb5"></asp:TextBox>
                    <small id="hint1">多个关键词用逗号“,”分隔。</small>
                </div>
    ...
    <script type="text/javascript">
        ...
        function SetVisible()
        {
            ...
            var rbl = document.getElementsByName("ctl00$ContentPlaceHolder1$rbtlMatchType");            
            for(var i =1;i<rbl.length;i++)
            {
                if(rbl[i].checked && rbl[i].value=="4")
                {
                    $("#ctl00_ContentPlaceHolder1_txtKeys").val("*");
                    $("#ctl00_ContentPlaceHolder1_txtKeys").attr("readonly","readonly");   
                    $("#hint1").html("“无匹配回复”默认关键词是*，不可修改");
                    $("#hint2").html("仅在未启动系统多客服设置的情况才生效 <a href=\"manyservice.aspx\">查看多客服设置>></a>");
                }else{
                    $("#ctl00_ContentPlaceHolder1_txtKeys").val("");
                    $("#ctl00_ContentPlaceHolder1_txtKeys").removeAttr("readonly");
                    $("#hint1").html("多个关键词用逗号“,”分隔。");
                    $("#hint2").html("精确匹配是对话完全一致才能触发，模糊匹配是包含就触发。");
                }
