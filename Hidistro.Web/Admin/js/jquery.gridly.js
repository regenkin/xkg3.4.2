...
function reSetimglist() {
    var attr = [];
    $('.gridly li.fileok').each(function () {
        $(this).attr('index', parseInt(parseInt($(this).css('top')) / 160 * 3 + parseInt($(this).css('left')) / 160 + 1));
        attr.push($(this));
    });
    attr.sort(function (a, b) {
        return a.attr('index') - b.attr('index');
    })
    $('.y-imglist li.img').each(function (i) {
        $(this).find('img').attr('src', attr[i].find('img').attr('src'));
    })
}