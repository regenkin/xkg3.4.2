<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/AdminNew.Master" AutoEventWireup="true" CodeBehind="EditShippingTemplate.aspx.cs" Inherits="Hidistro.UI.Web.Admin.Settings.EditShippingTemplate" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
                ...
                <div class="freight-template">
                    <div class="form-horizontal">
                        <div class="form-group">
                            <label for="inputEmail3" class="col-xs-2 control-label">当前模板ID</label>
                            <div class="col-xs-3">
                                <input type="Text" id="templateId" class="form-control">
                            </div>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-xs-3 setradiowidth" id="valuationmethod">
                                <span>
                                    <input type="radio" value="1" name="pric"  id="according" >
    ...
    <script type="text/javascript">
        ...
        function saveTemplates() {
            ...
            postData = { TemplateId: editTemplateId, task: "EditSave", Name: "", MUnit: 1, FreeShip: 0, HasFree: 0, shipperSelect: [], freeShippings: [] }; //要POST的数据

            if (postData.TemplateId == "") {
                HiTipsShow("读取模板ID失败，不能保存修改！", 'error')
                err = 1;
                return;
            }
                        ...
                        if ($RegionItems.length > 0) {
                            ...
                            $RegionItems.each(function () {
                                ...
                                if (! /(^\d+(\.\d{1,2})?$)/.test(SpecifyItem_Regions.FristNumber) || ! /(^\d+(\.\d{1,2})?$)/.test(SpecifyItem_Regions.AddNumber)) {
                                    ...
                                    if (err == 0) {
                                        HiTipsShow("[" + keyValue + "]指定地区运费单位请填写数值！", 'error')
                                        err = 1;
                                    }
                                    return;
                                };
            ...
            //使用AJAXA保存数据
            $.post("EditShippingTemplate.aspx", postData, function (msg) {
                if (msg != "") {

                    try {
                        var result = eval("(" + msg + ")");
                        if (result.state == "success" || result.msg == "模板修改成功！") {
                            HiTipsShow("添加修改成功,2秒后自动转到模板管理界面！", 'success', function () {
                                window.location = "ManageShippingTemplates.aspx";
                            });
                        } else {
                            HiTipsShow(result.msg, 'error');
                        }
        ...
        $(function () {
            ...
            $.getJSON("EditShippingTemplate.aspx", { task: "ReadData", TemplateId: editTemplateId, rand: Math.random() * 9999 }, function (tempData) {
                // alert(123);
                initEdit(tempData); //初始始化数据
                $("body").scrollTop(0);
            });
        })
        ...
        function initEdit(tempData) {
            ...
            if (tempData.TemplateId == null || tempData.TemplateId == "") {
          ...
          //  $("input[name=pric][value=" + tempData.MUnit + "]").trigger("");
            ...
            $.each(tempData.SpecifyRegionGroups, function (i) {

                if (this.IsDefault) {

                    $("input[name='ShippingType'][value=" + this.ModeId + "]").prop("checked", true);
                    $("input[name='ShippingType'][value=" + this.ModeId + "]").trigger("change"); //.prop("checked", true);

                    $thisNext = $("input[name='ShippingType'][value=" + this.ModeId + "]").parent().next(".freight-editor");
                    ...
                    var $editor = $("input[name='ShippingType'][value=" + this.ModeId + "]").parent().next();
                    ...
                    SelData["Group" + this.ModeId].Regions[row].RegionIds = this.RegionIds;

                    $RegionRow.find("p:first").attr("data-storage", this.RegionIds);
                    $RegionRow.find("p:first").html(getProvinces(this.RegionIds));
            ...
            $.each(tempData.FreeShippings, function (i) {
                ...
                if ($freerow.length > 0) {
                    ...
                    $freerow.find("p:first").attr("data-storage", this.RegionIds);
                    $freerow.find("p:first").html(getProvinces(this.RegionIds));
                    ...
                    SelData.FreeShippings[$freerow.index()].RegionIds = this.RegionIds;
