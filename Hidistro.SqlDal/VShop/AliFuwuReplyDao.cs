...
{
	public class AliFuwuReplyDao
	{
		private Database database;

		public AliFuwuReplyDao()
		{
		...
		{
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("DELETE vshop_AliFuwuReply WHERE ReplyId = @ReplyId;DELETE vshop_Message WHERE ReplyId = @ReplyId");
			this.database.AddInParameter(sqlStringCommand, "ReplyId", System.Data.DbType.Int32, id);
		...
		private bool SaveNewsReply(NewsReplyInfo model)
		{
			...
			stringBuilder.Append("insert into vshop_AliFuwuReply(");
		...
		public int GetNoMatchReplyID(int compareid)
		{
			...
			string query = "select top 1 ReplyId from vshop_AliFuwuReply where ReplyType=@ReplyType " + str + "  order by ReplyId desc";
		...
		{
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT COUNT(*) FROM vshop_AliFuwuReply WHERE Keys = @Keys");
		...
		public bool HasReplyKey(string key, int replyid)
		{
			...
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT COUNT(*) FROM vshop_AliFuwuReply WHERE Keys = @Keys " + str);
			this.database.AddInParameter(sqlStringCommand, "Keys", System.Data.DbType.String, key);
			return Convert.ToInt32(this.database.ExecuteScalar(sqlStringCommand)) > 0;
		}
